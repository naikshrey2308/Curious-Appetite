...
function CreateRecipeBasic(props) {
    ...
    async function Basics(event) {
        ...
        recipe.append("id", id);
        recipe.append("userId", user.id);
        recipe.append("categoryId", categoryId);
        recipe.append("name", name);
        recipe.append("description", description);
        recipe.append("preparationTime", prepTime);
        if(fileRef.current.files.length > 0)
            recipe.append("file", fileRef.current.files[0], fileRef.current.files[0].name);
        recipe.append("uploadDate", new Date().toGMTString());
        recipe.append("rating", 0);
        recipe.append("endNote", endnote);
        recipe.append("published", false);

        console.log(recipe);

        let res;
        if(id == 0) {
            res = await axios.post(`${Configuration.host}/api/Recipes`, recipe);
        } else {
            res = await axios.put(`${Configuration.host}/api/Recipes/${id}`, recipe);
        }
        console.log(res);
    ...
    return (
        <>
            ...
            <form className="d-flex align-items-center justify-content-evenly my-5" encType="multipart/form-data">
                ...
                <div style={{width: "30%"}}>
                    <img style={{objectFit: "cover", cursor: "pointer"}} ref={recipeImgRef} onClick={() => fileRef.current.click()} src="/images/profilepics/default-recipe.jpg" className="w-100 br-5 border" />
                ...
                <div>
                    <div className="position-relative">
                        <input ref={ingredientRef} type={"text"} className="mt-5 mb-3 mx-auto w-50 input-control form-control" placeholder="Type an ingredient" value={ingredient} onChange={changeIngredient} />
                        {
                            (searchBox) &&
                            <>
                                <div className="mx-auto w-50 search-under-box br-5 text-start position-absolute">
                                {(createBtn) && 
                    ...
                    steps.map((ele, ind) => {
                        return (
                            <div className={"d-flex container position-relative my-2 br-5 border justify-items-center"}>
                                ...
                                <img src={`${ele.stepImagePath}`} className="br-5 border m-2" width={100} />   
